...
function getPoint() {
	...
	var pricedz = $('#tid option:selected').attr('pricedz');
	var shopimg = $('#tid option:selected').attr('shopimg');
	var close = $('#tid option:selected').attr('close');
	$('#display_price').show();
	if(app_price!='undefined'){
		$('#display_app_price').show();
	}
	if(multi==1 && count>1){

		$('#need').val('￥'+price +"元 ➠ "+count+"个");

		$('#app_price').text(pricedz+'元');
	}else{
		$('#need').val('￥'+price +"元");
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				var num = 0;
				$.each(data.data, function (i, res) {
					$("#tid").append('<option value="'+res.tid+'" cid="'+res.cid+'" price="'+res.price+'" pricedz="'+res.pricedz+'" desc="'+escape(res.desc)+'" alert="'+escape(res.alert)+'" inputname="'+res.input+'" inputsname="'+res.inputs+'" multi="'+res.multi+'" isfaka="'+res.isfaka+'" count="'+res.value+'" close="'+res.close+'" prices="'+res.prices+'" max="'+res.max+'" min="'+res.min+'" stock="'+res.stock+'">'+res.name+'</option>');
					num++;
				});
