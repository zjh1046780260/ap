...
case 'gettool':
	if(isset($_POST['kw'])){
		...
		//$pric1=$rs['prid'];
        //$pricedz=$DB->getRow("SELECT * FROM pre_price WHERE id={$pric}");
...
case 'looknum':
    ...
    $tool=$DB->getRow("SELECT looknum FROM pre_tools WHERE tid='$tid'");
    $looknum1=$tool['looknum'];
    $lookznum=$looknum+$looknum1;
    $DB->exec("UPDATE pre_tools SET looknum='{$lookznum}' WHERE tid='$tid'");
    exit(json_encode($lookznum));
break;
